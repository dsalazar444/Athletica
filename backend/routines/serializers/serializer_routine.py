...
class RoutineCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if user and title:
            ...
            if qs.exists():
                raise serializers.ValidationError(f"Ya tienes una rutina con el titulo '{title}'. Ponle otro")
        return data
    ...
    def validate_assigned_athletes(self, value):
        ...
        for user in value:
            if getattr(user, 'role', None) != 'athlete':
                raise serializers.ValidationError(f"Usuario {user.username} no es un atleta. Solo se pueden asignar rutinas a usuarios Atletas")
        return value
