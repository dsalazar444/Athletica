...
class RoutineViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.created_by != request.user:
            return Response({"detail": "No tienes permiso para borrar esta rutina."}, status=status.HTTP_403_FORBIDDEN)
    ...
    def add_exercises(self, request, pk=None):
        ...
        current_max_order = routine.routine_exercises.aggregate(models.Max('order'))['order__max'] or 0
        new_exercises = [
            RoutineExercise(
                routine=routine,
    ...
    def assign_to_athletes(self, request, pk=None):
        ...
        if request.user.role != "coach":
            return Response({"detail": "Solo coaches pueden asignar."}, status=status.HTTP_403_FORBIDDEN)

        routine = self.get_object()
        athlete_ids = request.data.get("athlete_ids", [])
        if not athlete_ids:
            return Response({"detail": "Proporcione athlete_ids."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    @decorators.action(detail=False, methods=['get'], url_path='athlete/(?P<athlete_id>[^/.]+)/active')
    ...
    @decorators.action(detail=True, methods=['delete'], url_path='exercises/(?P<exercise_id>[^/.]+)')
    def remove_exercise(self, request, pk=None, exercise_id=None):
        """Quita un ejercicio de la rutina."""
        routine = self.get_object()
        deleted, _ = RoutineExercise.objects.filter(routine=routine, exercise_id=exercise_id).delete()
        if deleted:
...
class WorkoutSessionViewSet(viewsets.ModelViewSet):
    ...
    def history_range(self, request):
        ...
        sessions = self.get_queryset().filter(date__date__range=(start_date, end_date)).select_related("routine")
...
class SetLogViewSet(viewsets.ModelViewSet):
    ...
    @decorators.action(detail=False, methods=['get'], url_path='exercise/(?P<exercise_id>[^/.]+)/last')
    ...
    @decorators.action(detail=False, methods=['get'], url_path='exercise/(?P<exercise_id>[^/.]+)/history')
    def exercise_history(self, request, exercise_id=None):
        logs = SetLog.objects.filter(exercise_id=exercise_id).select_related("session").order_by("-session__date")
        history = {}
        for log in logs:
